...
export const fetchComments = async (unique_id, accessToken, page, callback) => {
  let data = await fetch(
    `http://192.168.20.107:8000/api/comment/list/${unique_id}?page=${page}`,
...
export const likeComment = async (unique_id, accessToken, callback) => {
  let response = await fetch(
    `http://192.168.20.107:8000/api/like/comment/create/${unique_id}`,
    {method: 'POST', headers: {Authorization: `Bearer ${accessToken}`}},
  );
};
...
export const postComment = async data => {
  let response = await fetch(
    `http://192.168.20.107:8000/api/comment/create/${data.unique_id}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
